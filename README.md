...
Each node in the tree is assigned to a specific index number. 

To access a node in the tree, assigned index values are used. Tree node values are stored in an indexed array. Child info of each node is stored in 2D array which holds index numbers of each child.
